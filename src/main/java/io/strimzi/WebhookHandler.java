...
/**
 * Handler processing webhook requests.
 */
public class WebhookHandler extends AbstractHandler {
    private final static Logger LOG = LoggerFactory.getLogger(WebhookHandler.class);
    ...
    private boolean drainKafka;
    private boolean drainZooKeeper;
    private boolean denyEviction;

    // Default constructor => used in production
    public WebhookHandler(KubernetesClient client) {
        this(client, 
            Configuration.STRIMZI_DRAIN_KAFKA, 
            Configuration.STRIMZI_DRAIN_ZOOKEEPER, 
            Configuration.STRIMZI_DENY_EVICTION);
    }

    // Parametrized constructor => used in tests
    public WebhookHandler(KubernetesClient client, boolean drainKafka, boolean drainZooKeeper, boolean denyEviction) {
        this.client = client;
    ...
    public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (body != null && !body.isBlank()) {
            AdmissionReview reviewRequest = Serialization.unmarshal(body, AdmissionReview.class);
            LOG.debug("Received AdmissionReview request: {}", reviewRequest);
            AdmissionReview reviewResponse = doHandle(reviewRequest);
            response.setContentType("application/json");
            response.setStatus(HttpServletResponse.SC_OK);
            response.getWriter().println(Serialization.asJson(reviewResponse));
        } else {
            LOG.warn("Weird, the request has an empty body");
            response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
        }
        baseRequest.setHandled(true);
    }

    /* test */ AdmissionReview doHandle(AdmissionReview admissionReviewRequest) {
        AdmissionRequest admissionRequest = admissionReviewRequest.getRequest();
        EvictionMetadata evictionMetadata = extractEvictionMetadata(admissionRequest);
        if (evictionMetadata != null) {
            String namespace = evictionMetadata.namespace();
            String name = evictionMetadata.name();
            if (namespace == null) {
                ...
                namespace = admissionRequest.getNamespace();
            }
            if (name == null || namespace == null) {
                ...
                if (pod != null) {
                    if (matchingLabel(pod.getMetadata().getLabels())) {
                        ...
                        if (matchingUuid(evictionMetadata.uid(), pod.getMetadata().getUid())) {
                            annotatePodForRestart(pod, admissionRequest.getDryRun());
    ...
    private EvictionMetadata extractEvictionMetadata(AdmissionRequest request) {
        if (request.getObject() instanceof Eviction eviction) {
            ...
            String uid = eviction.getDeleteOptions() != null && eviction.getDeleteOptions().getPreconditions() != null 
                ? eviction.getDeleteOptions().getPreconditions().getUid() : null;

            return new EvictionMetadata(eviction.getMetadata().getName(), eviction.getMetadata().getNamespace(), uid);
    ...
    private boolean matchingUuid(String evictionUuid, String podUuid) {
        if (evictionUuid == null || evictionUuid.equals(podUuid))   {
            ...
            LOG.warn("The UUID {} from the eviction request does not match the UUID of the current pod {}. " +
                "The request might be old and should be ignored.", evictionUuid, podUuid);
            return false;
        }
    }

    private void annotatePodForRestart(Pod pod, boolean dryRun) {
    ...
    private void patchPod(String name, String namespace, Pod pod, boolean dryRun)   {
        if (!dryRun) {
    ...
    private AdmissionReview denyRequest(AdmissionRequest request) {
        return new AdmissionReviewBuilder()
            ...
            .withStatus(new StatusBuilder()
                .withCode(500)
                .withMessage("The pod will be rolled by the Strimzi Cluster Operator")
                .build())
            .endResponse()
            .build();
    }
    ...
    record EvictionMetadata(String name, String namespace, String uid) { }
}
